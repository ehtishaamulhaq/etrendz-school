...
class Batch < ActiveRecord::Base
  ...
  def working_days(date)
    start=[]
    start<< self.started_on.to_date
    start<< date.beginning_of_month.to_date
    stop=[]
    stop<< self.ended_on.to_date
  ...
  def academic_days
    all_days=started_on.to_date..Date.today
    weekdays=Weekday.weekday_by_day(self.id).keys
    holidays=holiday_event_dates
    non_holidays=all_days.to_a-holidays
    range=non_holidays.select{|d| weekdays.include? d.wday}
  ...
  def gpa_enabled?
    Setting.has_gpa? and self.grading_type=="1"
  end

  def cwa_enabled?
    Setting.has_cwa? and self.grading_type=="2"
  end
  ...
  def subject_hours(starting_date,ending_date,subject_id)
    ...
    unless timetable_ids.nil?
      ...
      timetables.each do |tt|
        ([starting_date,start_date.to_date,tt.start_date].max..[tt.end_date,end_date.to_date,ending_date,Setting.default_time_zone_present_time.to_date].min).each do |d|
  ...
  def perform
    ...
    prev_record = Setting.find_by_config_key("job/Batch/#{self.job_type}")
    if prev_record.present?
      prev_record.update_attributes(config_value: Time.now)
    else
      Setting.create( config_key: "job/Batch/#{self.job_type}", config_value: Time.now)
    end
  end
