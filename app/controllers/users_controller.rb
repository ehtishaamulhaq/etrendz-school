class UsersController < ApplicationController
  ...
  def search_user_ajax
    unless params[:query].nil? or params[:query].empty? or params[:query] == ' '
       @user = User.active.where("first_name LIKE ? OR last_name LIKE ? OR username LIKE  ?", "%#{params[:query]}%", "%#{params[:query]}%", "%#{params[:query]}%")
    else
      @user = ''
    end
  ...
  def list_user
    if params[:user_type] == 'Admin'
      ...
      #   end
      # end
      @emp_dept = EmployeeDepartment.active
      render partial: 'employee_user'
  ...
  def dashboard
    ...
    @first_time_login = Setting.get_config_value('FirstTimeLoginEnable')
  ...
  def forgot_password
    ...
    @network_state = Setting.find_by_config_key("NetworkState")
  ...
  def login
    @institute = Setting.find_by_config_key("LogoName")
    ...
    # else
      if request.post? and params[:user]
        ...
        user = User.active.find_by_username params[:user][:username]
        if user.present? and User.authenticate?(@user.username, params[:user][:password])
  ...
  def first_login_change_password
    @user = User.active.find_by_username(params[:id])
    @setting = Setting.get_config_value('FirstTimeLoginEnable')
  ...
  def backup
    ...
    result = system "mysqldump --opt --user=#{username} --password=#{password} #{database} > dbs/school_#{Time.now.strftime('%Y%m%d%H%M%S')}.sql"
    if result
      redirect_to '/', notice: 'Database was successfully Backedup.'
    else
  ...
  def successful_user_login(user)
    ...
    redirect_to :back || {:controller => 'users', :action => 'dashboard'}
  end


  def user_params
    params.require(:user).permit(:username, :first_name, :last_name, :email, :admin, :student, :employee, :hashed_password) 
  end
end
