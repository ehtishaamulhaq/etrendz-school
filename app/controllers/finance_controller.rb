class FinanceController < ApplicationController
  ...
  def update_monthly_report
    ...
    @other_transaction_categories = FinanceTransaction.where.not(category_id: @fixed_cat_ids).where("transaction_date >= ? and transaction_date <= ?", @start_date, @end_date).order(:transaction_date).page(params[:page]).pluck(:category_id)
    @other_transaction_categories = FinanceTransactionCategory.find(@other_transaction_categories)  # THIS IS A TEMPORARY FIX FOR THE ABOVE QUERY
  ...
  def master_category_create
    if request.post?
      ...
      respond_to do |format|
        ...
        # format.json { render @finance_fee_category }
      end
    end
  end
  ...
  def fee_collection_create
    ...
    if request.post?
      unless params[:fee_collection].nil?
        ...
        subject = "#{t('fees_submission_date')}"
        p "===================="
        p params[:finance_fee_collection][:start_date]
        p (params[:finance_fee_collection][:start_date]).to_s.to_date
        p "+++++++++++++++++++"
        category.each do |c|
          ...
          @finance_fee_collection = FinanceFeeCollection.new(
            ...
            :start_date => params[:finance_fee_collection][:start_date].to_s.to_date,
            :end_date => params[:finance_fee_collection][:end_date].to_s.to_date,
            :due_date => params[:finance_fee_collection][:due_date].to_s.to_date,
            :fee_category_id => c,
            :batch_id => b.id
          )
          if @finance_fee_collection.save
            @students = Student.where(batch_id: b.id)
  ...
  def update_fees_collection_dates
    ...
    # end
    respond_to do |format|
      format.js { render 'fees_collection_dates'}
    end
  end
  ...
  def load_fees_submission_batch
    ...
    unless @fee.nil?
      ...
      unless @financefee.transaction_id.blank?
        ...
        @paid_fees = FinanceTransaction.find(@financefee.transaction_id.split(','))
      end
      ...
      respond_to do |format|
        format.js { render 'student_fees_submission'}
      end
      ...
      render :text => '<p class="flash-msg">No students have been assigned this fee.</p>'
  ...
  def fees_student_dates
    @student = Student.find(params[:id])
    @dates = @student.batch.fee_collection_dates.to_a
  ...
  def fees_submission_student
    ...
    @due_date = @fee_collection.due_date
    @fee_category = FinanceFeeCategory.where("is_deleted is not null").find(@fee_collection.fee_category_id)
    @fee_particulars = @date.fees_particulars(@student)
    unless @financefee.transaction_id.blank?
      @paid_fees = FinanceTransaction.where("FIND_IN_SET(id,\"#{@financefee.transaction_id}\")")
    end
  ...
  def fees_submission_save
    ...
    respond_to do |format|
      format.js { render 'fees_submission_student'}
    end
  end
