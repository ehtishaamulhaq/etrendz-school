class ExamGroupsController < ApplicationController
  ...
  def exam_group_params
    params.require(:exam_group).permit(:start_date, :end_date, :subject_id, :name, :exam_type, :exams_attributes => [:id, :subject_id, :start_time, :end_time, :maximum_marks, :minimum_marks, :grading_level_id, :weightage, :event_id]) if params[:exam_group]
  end
end