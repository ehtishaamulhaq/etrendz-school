...
class EmployeeController < ApplicationController
  ...
  def add_additional_details
    ...
    if request.post?
      ...
      @additional_field = AdditionalField.new(additional_field_params)
  ...
  def admission1
    ...
    @config = Setting.find_by_config_key('EmployeeNumberAutoIncrement')
  ...
  def admission2
    ...
    @employee = Employee.find(params[:id])
    @selected_value = Setting.default_country
    if request.post? and @employee.update_attributes(employee_params)
  ...
  def admission3_1
    ...
    if request.post?
      @error=false
      mandatory_fields = AdditionalField.where('is_mandatory = ? and status = ?', true, true)
      ...
      unless @error==true
        ...
        unless params[:edit_request].present?
          flash[:notice] = "#{t('employee.flash25')}#{@employee.first_name}"
          redirect_to :action => "edit_privilege", :id => @employee.employee_number
        else
  ...
  def edit_privilege
    ...
    @finance = Setting.find_by_config_value("Finance")
  ...
  def select_reporting_manager
    other_conditions = ""
    other_conditions += " AND employee_department_id = '#{params[:employee_department_id]}'" unless params[:employee_department_id].blank?
    other_conditions += " AND employee_category_id = '#{params[:employee_category_id]}'" unless params[:employee_category_id].blank?
    other_conditions += " AND employee_position_id = '#{params[:employee_position_id]}'" unless params[:employee_position_id].blank?
    other_conditions += " AND employee_grade_id = '#{params[:employee_grade_id]}'" unless params[:employee_grade_id].blank?
    @employee = Employee.where("(first_name LIKE ? OR middle_name LIKE ? OR last_name LIKE ?
                       OR employee_number = ?)  #{other_conditions}",
          "#{params[:query]}%","#{params[:query]}%","#{params[:query]}%",
          "#{params[:query]}",).order("employee_department_id asc,first_name asc") unless params[:query].blank?
  ...
  def profile_payroll_details
    @currency_type = Setting.find_by_config_key("CurrencyType").config_value
    @employee = Employee.find(params[:id])
  ...
  def create_monthly_payslip
    ...
    @dependent_categories = PayrollCategory.where("status = ? and payroll_category_id is not NULL ", true)
    @employee_additional_categories = IndividualPayslipCategory.where('employee_id = ? and include_every_month = ?', @employee.id, true)
    @new_payslip_category = IndividualPayslipCategory.where('employee_id = ? and salary_date is ? ', @employee.id,nil)
    ...
    if request.post?
      ...
      unless salary_date.to_date < @employee.joining_date.to_date
        ...
        if payslip_exists == []
          ...
          flash[:notice] = "#{@employee.first_name} #{t('employee.flash28')} #{salary_date}"
        end
