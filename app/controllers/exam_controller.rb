...
class ExamController < ApplicationController
  ...
  def generated_report
    ...
    if params[:student].nil?
      ...
      @graph = OpenFlashChartObject.open_flash_chart_object(770, 350, "/exam/graph_for_generated_report?batch=#{@student.batch.id}&examgroup=#{@exam_group.id}&student=#{@student.id}")
    else
      ...
      if request.xhr?
        ...
        respond_to do |format|
          format.js { render 'generated_report' }
        end
      else
  ...
  #   else
  #     if Date.civil(*params[:attendance_rank][:started_on].sort.map(&:last).map(&:to_i)) > Date.civil(*params[:attendance_rank][:ended_on].sort.map(&:last).map(&:to_i))
  #       flash[:notice] = "#{t('flash15')}"
  #       redirect_to :action=>'attendance_rank' and return
  #     else
  #       @batch = Batch.find(params[:attendance_rank][:batch_id])
  #       @students = Student.where(batch_id: @batch.id)
  #       @start_date = Date.civil(*params[:attendance_rank][:started_on].sort.map(&:last).map(&:to_i))
  #       @end_date = Date.civil(*params[:attendance_rank	][:ended_on].sort.map(&:last).map(&:to_i))
  ...
  def student_attendance_rank
    if params[:attendance_rank].nil? or params[:attendance_rank][:batch_id].empty?
      ...
      if params[:attendance_rank][:started_on].to_s.to_date > params[:attendance_rank][:ended_on].to_s.to_date
        flash[:notice] = "#{t('flash15')}"
        redirect_to :action=>'attendance_rank' and return
      else
        @batch = Batch.find(params[:attendance_rank][:batch_id])
        @students = Student.where(batch_id: @batch.id)
        @start_date = params[:attendance_rank][:started_on].to_s.to_date
        @end_date = params[:attendance_rank][:ended_on].to_s.to_date
        @ranked_students = @batch.find_attendance_rank(@start_date,@end_date)
      end
    end
  end
  ...
  def student_attendance_rank_pdf
    @batch = Batch.find(params[:batch_id])
    @students = Student.where(batch_id: @batch.id)
    @start_date = params[:started_on].to_date
    @end_date = params[:ended_on].to_date
    @ranked_students = @batch.find_attendance_rank(@start_date,@end_date)
