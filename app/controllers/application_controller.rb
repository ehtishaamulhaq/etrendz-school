...
class ApplicationController < ActionController::Base
  ...
  before_filter :set_title   # ADDED BY EHTI AFTER COMMENTING INITIALIZE AS IT CAUSED LAYOUT RENDER FAILURE
  ...
  def set_variables
    unless @current_user.nil?
      @attendance_type = Setting.get_config_value('StudentAttendanceType') unless @current_user.student?
      @modules = Setting.available_modules
    end
  end
...
#   def initialize
#     p "==============================="
#     p FedenaSetting.company_details[:company_name]
# #    @title = FedenaSetting.company_details[:company_name]
#   end
  ...
  def configuration_settings_for_hr
    hr = Setting.where(config_value: "HR").first
    if hr.nil?
      redirect_to dashboard_users_path, notice: t('flash_msg4')
    end
  end
...
#       end
  ...
  def set_user_language
    lan =  {} #Setting.find_by_config_key("Locale")
